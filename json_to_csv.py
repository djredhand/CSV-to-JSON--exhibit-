...
def main():
    ...
    data = json.load(open(filename, 'U'))
    csvw = csv.writer(open('new_csv.csv', 'w+b') )
    csvw.writerow(['Comp01','Comp01Level','Comp02','Comp02Level','Comp03',
    ...
    try:
        for i, items in enumerate(data['items']):
            count += 1
            print data['items'][i]['competency_domain']

            csvw.writerow([data['items'][i]['Comp01'],
